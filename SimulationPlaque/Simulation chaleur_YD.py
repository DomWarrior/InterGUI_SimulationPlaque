...
def vector_evolution_temperature(T, pos_ac, nx_ac, ny_ac, P_ac=None,
    ...
    # Convection
    Coeff = (h*dt)/(p*cp)
    T_new[0, :] += 0*Coeff*(T_air-T_new[0,:])*((dz*dx)/(vol))     # haut   #ici, j'ai mis un coefficient 1 parce que si je veux enlever la convection juste sur 1 cote, il me suffira juste de rempacer 1 par 0
    T_new[-1, :] += 0*Coeff*(T_air-T_new[-1,:])*((dz*dx)/(vol))   # bas
    T_new[:,0] += 0*Coeff*(T_air-T_new[:,0])*((dz*dy)/(vol))      # gauche
    T_new[:,-1] += 0*Coeff*(T_air-T_new[:,-1])*((dz*dy)/(vol))    # droite
    T_new[:,:] += 0*Coeff*(T_air-T_new[:,:])*((dx*dy)/vol)      # dessus/dessous  #ici, le coefficient doit etre a 2 si on veut que les 2 surfaces soient soumises a la convection
...
def animate_3D():
    ...
    global surf     
    surf = ax.plot_surface(X, Y, T, cmap='jet', vmin=T_min, vmax=T_max)
    ...
    # Tracés des températures des trois thermistances
    ax2.plot(times, temp_therm_1, marker='o', linestyle='-', color='r', label='Thermistance 1 (y=80)')   #affiche la température de la thermistance où l'actuateur
    ax2.plot(times, temp_therm_2, marker='s', linestyle='--', color='y', label='Thermistance 2 (centre)')  #affiche la température de la 2e thermistance.
    ax2.plot(times, temp_therm_laser, marker='^', linestyle='-.', color='k', label='Thermistance laser (y=n_y/4)')   #affiche la température de la thermistance où le laser.
    ax2.set_xlabel("Temps (s)")
    ax2.set_ylabel("Température (K)")
    ax2.set_title("Évolution des températures dans la plaque")
    ax2.legend()
    ...
    fig3, ax3 = plt.subplots()
    ax3.plot(times, Energie_list, marker='o')
    ax3.set_xlabel("Temps (s)")
    ax3.set_ylabel("Energie interne plaque(J)")
    ax3.set_title("Évolution de l'energie interne")
    plt.show()
