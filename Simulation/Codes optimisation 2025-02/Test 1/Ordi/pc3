...
temp_therm_1_ref, temp_therm_2_ref, temp_therm_laser_ref = Actu[1:50000], T2[1:50000], Laser[1:50000] 
...
h_values = np.arange(5,25,1)
P_values = np.arange(0.5, 2, 0.1)
errors = []
...
for P in P_values:
    for h in h_values:
        ...
        print(len(temp_therm_1_test), len(Actu[1:]))
        ...
        errors.append(error)
        print(errors)
        print(f'Fin simulation pour ({h}:{P})')
...
temp_therm_1_best = temp_therm_1_best[::3]
temp_therm_2_best = temp_therm_2_best[::3]
temp_therm_laser_best = temp_therm_laser_best[::3]
...
plt.plot(times, temp_therm_2_best, linestyle='--', color='r', 
         label=f'Test Thermistance 2 (h={Hp_optimal[0]:.2f}, P={Hp_optimal[1]:.2f})')


# Thermistance 3
plt.plot(times, temp_therm_laser_ref, linestyle='-', color='k', label='Réf Thermistance Laser')
plt.plot(times, temp_therm_laser_best, linestyle='--', color='r', 
         label=f'Test Thermistance laser (h={Hp_optimal[0]:.2f}, P={Hp_optimal[1]:.2f})')
